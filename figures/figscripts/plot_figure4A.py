...
def plot_luc_data_lower(df, dfout):
	fig, ax = plt.subplots(figsize=(6,3))

	xVar = 'StopCodon'
	yVar = 'Ratio_G418_Norm'
	yVar2 = 'ratio_mean_G418_Norm'
	yErr = 'ratio_sd_G418_Norm'

	sns.swarmplot(data=df, x=xVar, y=yVar, size=5, color= colorDict[ 'orange'], edgecolor='black', linewidth=0.5)
	...
	plt.savefig("%s/figures/Fig4A_lower.pdf" % rootDir, format="pdf", bbox_inches = "tight")


def plot_luc_data_upper(df, dfout):
	...
	plt.savefig("%s/figures/Fig4A_upper.pdf" % rootDir, format="pdf", bbox_inches = "tight")

def main():
	df = input_luc_data()
	dfout, minVal = calc_luc_ratios(df)
	df = adjust_ratio(df, minVal)
	plot_luc_data_lower(df, dfout)
	plot_luc_data_upper(df, dfout)
