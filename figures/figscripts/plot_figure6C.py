...
###
print samplelist
# samples = [samplelist[27]]+ [samplelist[28]]
samplelist = [samplelist[2]]+ [samplelist[3]]
# print samples
...
def plot_genemodels_combined(df_all, UTRdict, coloR, trspname, samp, treat):
	...
	plotoutfile = "%s/figures/Fig6C_%s.pdf" % (rootDir, treat)
	...
	for i in range(len(df_all.columns)):
		...
		dfToPlot.plot.line(y='rpm', use_index=True, ax = axes, c=coloR, linewidth = 0.75)
...
def main():
	...
	# print df_all
		plot_genemodels_combined(df_all, UTRdict, coloR, trspname, samp, treat)
