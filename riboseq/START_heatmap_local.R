...
dtF= dtT[,cutrange]
write.table(dtF,"biglist_F.csv", row.names= TRUE, col.names= FALSE, sep=",")
dtW= read.csv("biglist_F.csv", header= TRUE, sep= ",")
...
plot= ggplot(dt2.m, aes(variable, pos)) + 
  ...
  scale_x_discrete(name= "5'ends of reads to START", labels= c(cutrange-length(cutrange)))+
