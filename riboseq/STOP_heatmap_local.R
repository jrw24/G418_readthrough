...
for (ft in ftsize) 
{
  infile= sprintf("%s_%sf_avg_2.csv",fname,ft)
...
cutrange = 120:180
dtF= dtT[,cutrange]
write.table(dtF,"biglist_F_stop.csv", row.names= TRUE, col.names= FALSE, sep=",")
dtW= read.csv("biglist_F_stop.csv", header= TRUE, sep= ",")
...
plot= ggplot(dt2.m, aes(variable, pos)) + 
  ...
  scale_x_discrete(name= "5'ends of reads to STOP", labels= c(cutrange-length(cutrange)))+
  scale_y_discrete(name= "Footprint size", labels= c(ftsize))
...
ggsave(file= output, heatmap, width= 5, height= 3, units='in', dpi= 600, pointsize= 0.5)
plot(heatmap)
dev.off()